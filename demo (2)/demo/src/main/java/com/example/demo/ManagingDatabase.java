...
public class ManagingDatabase {
    private static final String JDBC_URL = "jdbc:mysql://localhost:3306/AssignmentQuestion2";
    ...
    public static void insertData(List<DataModel> data) {
        data.parallelStream().forEach(record -> {
            try (Connection connection = dataSource.getConnection();
                ...
                preparedStatement.setString(8, record.getCurrentLoc());
                preparedStatement.setString(9, record.getPreferredLoc());
    ...
    public static void teamWiththeMaximumInterviews() {
        try (Connection connection = dataSource.getConnection();
             Statement statement = connection.createStatement()) {
    ...
    public static void teamWiththeMinimumInterviews() {
        try (Connection connection = dataSource.getConnection();
             Statement statement = connection.createStatement()) {
